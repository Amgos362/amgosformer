...
####################################
def encode_datetime_features_onehot(data, dim=100):
    if 'datetime' not in data.columns:
        data['datetime'] = pd.to_datetime(data.index)
    
    # hour 정보가 있는지 검사 (유일값이 1개면 제외)
    if data['datetime'].dt.hour.nunique() > 1:
        data['hour_of_day'] = data['datetime'].dt.hour
    else:
        data['hour_of_day'] = np.nan
    ...
    def onehot_with_fixed_dim(series, prefix, dim):
        if series.isna().all():
            return pd.DataFrame(index=series.index)  # 인덱스를 그대로 유지
    ...
    # 원본 데이터와 one-hot 인코딩된 컬럼을 모두 포함하도록 합침
...
# 추가: Order(순위) 인코딩 함수
####################################
def add_order_encoding(data, group_cols, new_prefix):
    # 행별 순위 계산 (낮은 값에 낮은 순위)
    rank_df = data[group_cols].rank(method='min', axis=1)
    for col in group_cols:
        data[new_prefix + '_' + col + '_order'] = rank_df[col]
    return data
...
# 5. 데이터 로드 및 전처리
####################################
data = pd.read_csv("AAPL_price.csv", index_col=0)
...
data = encode_datetime_features_onehot(data, dim=100)
...
ohcl_cols = ['open', 'high', 'low', 'close']
data = add_order_encoding(data, ohcl_cols, new_prefix='OHLC')
data = onehot_order_encoding(data, ['OHLC_' + col + '_order' for col in ohcl_cols], num_categories=4)
# SMA 순위 (SMA_5, SMA_10, SMA_20, SMA_60, SMA_120, SMA_250) → 6 범주
sma_cols = ['SMA_5', 'SMA_10', 'SMA_20', 'SMA_60', 'SMA_120', 'SMA_250']
data = add_order_encoding(data, sma_cols, new_prefix='SMA')
data = onehot_order_encoding(data, ['SMA_' + col + '_order' for col in sma_cols], num_categories=6)
