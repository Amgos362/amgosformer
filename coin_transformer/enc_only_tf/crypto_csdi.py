...
class CSDIDiffusion(nn.Module):
    def __init__(self, input_dim, d_model=128, num_heads=8, num_layers=3, num_timesteps=100):
        ...
        # Diffusion parameters (following CSDI paper)
    ...
    def add_noise(self, x, t):
        """Add CSDI-style continuous noise"""
        noise = torch.randn_like(x)
        
        # Ensure t is properly shaped for broadcasting
        sqrt_alphas = self.sqrt_alphas_cumprod[t].view(-1, 1)
        sqrt_1m_alphas = self.sqrt_1m_alphas_cumprod[t].view(-1, 1)
        
        return sqrt_alphas * x + sqrt_1m_alphas * noise, noise
    ...
    def sample(self, x_condition, device):
        ...
        for t in reversed(range(self.num_timesteps)):
            ...
            with torch.no_grad():
                ...
                # Update sample
                y = (1 / torch.sqrt(1 - beta)) * (y - beta * predicted_noise / torch.sqrt(1 - alpha)) + \
...
def prepare_csdi_data(data_path: str, lookback: int = 24) -> Tuple[pd.DataFrame, pd.Series]:
    """Prepare data for CSDI model with binning"""
...
def train_csdi_model(model, train_loader, val_loader, num_epochs, device, patience=5, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            # Add CSDI noise
            y_noisy, noise = model.add_noise(y, t)
...
def main():
    ...
    X, y = prepare_csdi_data('BTC_upbit_KRW_min60.csv')
    ...
    for window_idx, ((X_train, y_train), (X_val, y_val), (X_test, y_test)) in enumerate(splits):
        ...
        # Create datasets
        train_dataset = CSDITimeSeriesDataset(X_train, y_train)
        val_dataset = CSDITimeSeriesDataset(X_val, y_val)
        test_dataset = CSDITimeSeriesDataset(X_test, y_test)
        ...
        if window_idx > 0:
            prev_model_path = f'csdi_model_window_{window_idx}.pth'
            try:
        ...
        # Evaluate model
        test_mse, test_accuracy, predictions, true_values = evaluate_csdi_model(
            model,
            test_loader,
            device
        )
        ...
        print(f"Direction Accuracy: {test_accuracy:.4f}")
        
        window_accuracies.append(test_accuracy)
        
        # Save model for this window
        torch.save(model.state_dict(), f'csdi_model_window_{window_idx + 1}.pth')
    
    # Calculate and print average accuracy across all windows
    avg_accuracy = sum(window_accuracies) / len(window_accuracies)
    print(f"\nFinal Results:")
    print(f"Average direction accuracy across all windows: {avg_accuracy:.4f}")
